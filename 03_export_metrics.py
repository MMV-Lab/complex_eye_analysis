...
from metrics import filter_tracks, calculate_speed, calculate_track_duration
...
pixel_edge_length_per_frame_in_µm_per_minute = (
    pixel_edge_length_to_µm / frameperiod_minutes
)  # [µm/minute]
...
mean_track_duration = []
mean_track_duration.append("mean_track_duration [s]")
std_track_duration = []
std_track_duration.append("std_duration [s]")

### get metrics
for movie in movies:
    tracks = np.load(
        "./data/tracks/" + movie.replace(".tiff", "") + "_trackslayer.npy"
    ).astype(int)
    ...
    amount_total_cells = amount_total_tracks - sum(
        np.unique(tracks[:, 0], return_counts=True)[1]
        < min_track_duration_to_be_considered_as_a_cell
    )
    ...
    moving_cells_perc.append(
        np.round(amount_moving_cells / amount_total_cells * 100, 3)
    )

    wells.append(movie.replace(".npy", ""))
    min_track_to_be_cell.append(
        min_track_duration_to_be_considered_as_a_cell * frameperiod
    )
    frame_periods.append(frameperiod)
    microscopic_resolutions.append(microscopic_resolution)

    track_duration = calculate_track_duration(filtered_tracks)
    mean_track_duration.append(track_duration[0] * frameperiod)
    std_track_duration.append(track_duration[1] * frameperiod)
